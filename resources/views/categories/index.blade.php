...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group pull-right">
            <a href="{{ URL('admin/categories/create') }}" id="sample_editable_1_new" class="btn green"> Add New
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption font-dark">
                <i class="icon-categories font-dark"></i>
        ...
        <div class="portlet-body">
            <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="sample_1">
                ...
                <tbody>
                    ...
                    <?php $post = makeDefaultImage($post, 'categories') ?>
                    <tr id="categories-{{ $post->id }}">
                        <td>{{ $x }}</td>
                        <td>{{ $post->name_en }}</td>
                        <td>{{ $post->name_ar }}</td>
                        <td>{{ $post->desc_en }}</td> 
                        <td>{{ $post->desc_en }}</td>                            
                        <td><div>{!!$post->image ? '<img src="../uploads/category/'.$post->image.'" height="40"/>':''!!}</div> </td>                          


<!--                            <td class="text-center">
                               <div class="checkbox-nice checkbox-inline">
                                   <input data-id="{{ $post->id }}" type="checkbox" @if(!PerUser('categories_active')) disabled="disabled" @endif class="@if(PerUser('categories_active')) changeStatues @endif" @if($post->active==1) checked="checked" @endif id="checkbox-{{ $post->id }}">
                        ...
                        <td>
                            <div class="btn-group pull-right">
                                ...
                                <ul class="dropdown-menu pull-right">
                                    @if(PerUser('categories_edit'))
                                    <li>
                                        <a href="{{ URL('admin/categories/'.$post->id.'/edit') }}">
