...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.profiles') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('profiles_add'))
    <div class="row">
        <div class="col-md-12">
            <div class="btn-group pull-right">
                <a href="{{ URL('admin/profiles/create') }}" id="sample_editable_1_new" class="btn green"> Add New
            ...
            <div class="portlet-body">
                <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="sample_1">
                    ...
                    @foreach($profiles as $post)
                        ...
                        <tr id="profiles-{{ $post->id }}">
                            <td>{{ $x }}</td>
                            <td>{{ $post->name }}</td>
                            <td class="text-center">
                                <div class="checkbox-nice checkbox-inline">
                                    <input data-id="{{ $post->id }}" type="checkbox" @if(!PerUser('profiles_active')) disabled="disabled" @endif class="@if(PerUser('profiles_active')) changeStatues @endif" @if($post->active==1) checked="checked" @endif id="checkbox-{{ $post->id }}">
                            ...
                            <td>
                                <div class="btn-group pull-right">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        @if(PerUser('profiles_edit'))
                                        <li>
                                            <a href="{{ URL('admin/profiles/'.$post->id.'/edit') }}">
                                                <i class="fa fa-pencil"></i> {{ Lang::get('main.edit') }} </a>
                                        </li>
                                        @endif
                                        @if(PerUser('profiles_delete'))
                                        <li>
        ...
        $(document).ready(function(){
            ...
            $(document).on('change','.changeStatues',function(){
                ...
                if(statues){
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/profiles/activation') }}",
            ...
            $(document).on('click','.delete_this',function(event){
                ...
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.profiles') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.profiles') }} ?',
                    buttons: [
                        {
                            ...
                            action: function(dialogItself){
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/profiles') }}/"+deleted_id,
